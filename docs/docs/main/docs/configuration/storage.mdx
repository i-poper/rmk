...
::: info Flashing a new firmware can erase stored data

Changing the RMK version, commit or features erases everything, BLE pairings included. Changing only your layout preserves stored values and pairings. Set `clear_layout = true` to replace the stored layout with the firmware's defaults while keeping pairings. See [Storage](/docs/features/storage) for the full table.
